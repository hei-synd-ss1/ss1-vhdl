...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.tx_module_menu = tk.OptionMenu(self.tx_gridframe, self.tx_module, "DC Motor", "Direction Motor", "Sensors", "CReg")
        ...
        self.test_dmot_button = tk.Button(self.test_frame, text="Direction Motor", command=self.move_direction)
        self.test_dmot_button.grid(row=0, column=2, sticky='nesw', padx=5)#.pack(side="left", fill='x', padx=5)
    ...
    def move_direction(self):
        self.test_buttons_states(False)
        self.send_n_write(0x60, 127, "Direction test\n   Servo to 127 (middle)", 3000)
        self.send_n_write(0x60, 0, "   Servo to 255", 3000)
        self.send_n_write(0x60, 0, "   Servo to 0", 3000)
        self.send_n_write(text = "Direction test done\n")
        self.test_buttons_states(True)
    ...
    def send_data(self, isRead : bool):
        ...
        if module == "DC Motor":
            ...
            if register == "Prescaler":
                ...
                return
        elif module == "Direction Motor":
            module = 0x40 if isRead else 0x60
            if register == "Target Command":
                pass
            elif register == "Custom Register 1":
                module += 1
            elif register == "Custom Register 2":
                module += 2
            elif register == "Custom Register 3":
                module += 3
        elif module == "Sensors":
    ...
    def update_tx_module(self, *args):
        ...
        if self.tx_module.get() == "DC Motor":
        ...
        elif self.tx_module.get() == "Direction Motor":
            self.tx_register_menu['menu'].add_command(label="Target Command", command=lambda: self.tx_register.set("Target Command"))
            self.tx_register_menu['menu'].add_command(label="Custom Register 1", command=lambda: self.tx_register.set("Custom Register 1"))
            self.tx_register_menu['menu'].add_command(label="Custom Register 2", command=lambda: self.tx_register.set("Custom Register 2"))
            self.tx_register_menu['menu'].add_command(label="Custom Register 3", command=lambda: self.tx_register.set("Custom Register 3"))
            self.tx_register.set("Target Command")
        elif self.tx_module.get() == "Sensors":
            for i in range(NB_LEDS):
                self.tx_register_menu['menu'].add_command(label="LED/SERVO " + str(i+1), command=lambda i=i: self.tx_register.set("LED/SERVO " + str(i+1)))
            ...
            self.tx_register.set("LED/SERVO 1")
    ...
    def update_data_labels(self):
        ...
        if self.uart_handler is not None:
            ...
            if rxd[1] >= 0 and rxd[1] < 0x40:
                ...
                t += "Direction Motor "
                if r == 0:
                    t += " | Target Command | " + str(rxd[2] << 8 | rxd[3])
                elif r == 1:
                    t += " | Custom Register 1 | " + str(rxd[2] << 8 | rxd[3])
                elif r == 2:
                    t += " | Custom Register 2 | " + str(rxd[2] << 8 | rxd[3])
                elif r == 3:
                    t += " | Custom Register 3 | " + str(rxd[2] << 8 | rxd[3])
                else:
                    t += " | Unknown"
            elif rxd[1] >= 0x80 and rxd[1] < 0xC0:
                t += "Sensors "
                if r >= 0 and r < NB_LEDS:
                    t += " | Led " + str(r+1) + " | " + ("on" if rxd[2] & 0x80 else "off") + " - period of " + str((rxd[2] & 0x7F) << 8 | rxd[3]) + " ms / Servo " + str(r+1) + " | " + str(((rxd[2] & 0x7F) << 8 | rxd[3]) / 10000) + "ms on pulse"
                ...
                t += "Control Registers "
                if r == 0:
                    t += " | Hardware Control | " + ("forwards" if rxd[3] & 0x01 else "backwards")
                elif r == 1:
