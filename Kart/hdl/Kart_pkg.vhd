...
-- Sensors
  ...
  constant DMOT_MINCMD_uS : positive := 500; -- MUST correspond to servomotor minimal pulse time to avoid it burning
  constant DMOT_MAXCMD_uS : positive := 2500; -- MUST correspond to servomotor maximal pulse time to avoid it burning
  ...
  constant DMOT_MAXCMD_CLOCKS : positive := positive(real(DMOT_MAXCMD_uS) * 1.0E-6 * CLOCK_FREQUENCY);
    -- may be bigger than DMOT_MAXCMD_CLOCKS, but will be caught by dmotServoPulseValidator
  constant DMOT_CMD_CLOCKS_STEP : positive := positive((real(DMOT_MAXCMD_CLOCKS - DMOT_MINCMD_CLOCKS) / real(2**DMOT_TARGETCMD_BIT_NB - 1)) + 1.0);
