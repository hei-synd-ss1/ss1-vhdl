...
ARCHITECTURE rtl OF txRouter IS
  ...
  signal p_dmotAuth, p_dcAuth, p_sensorsAuth, p_cregAuth, p_load : std_ulogic;
    ...
    if reset = '1' then
      ...
      case p_state is
        when idle =>
          p_load <= '0';
          -- Sorted by priority
          testAndSend(dmotAddressToSend, dmotDataToSend,
            dmotSendRequest, p_dmotAuth, p_addr, p_data, p_state, full, sent);
          if sent = '0' then
